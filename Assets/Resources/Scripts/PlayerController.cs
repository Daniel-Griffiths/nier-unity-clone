...
public class PlayerController : MonoBehaviour
{
    ...
    void Shoot()
    {
        GameObject bullet = Resources.Load("Prefabs/Hacking/PlayerBullet") as GameObject;

        _audioSource.PlayOneShot(shootSound, 0.75f);

        // We spawn the bullet slightly higher, otherwise it collides with the ground and instantly despawns.
        Vector3 bulletSpawnPosition = new Vector3(transform.position.x, transform.position.y + .4f, transform.position.z);

        Instantiate(bullet, bulletSpawnPosition, transform.rotation).GetComponent<Rigidbody>()
